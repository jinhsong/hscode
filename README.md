...
## v5.1 — CBP 장애가 예산을 독식하던 문제 해결

실행 로그 분석: CBP 요청 56건이 각 ~25초 지연 후 500으로 죽으며 혼자 217초(예산 80%)를 소모 →
RSS 절반·Gemini 5개 패스·검증·번역이 전부 생략됐던 문제.

- **CBP 프로브 방식**: 4개 파라미터 변형을 1건짜리 병렬 프로브로 먼저 확인 → 유효 조합으로만
  본 수집(성공 조합 캐시), 전부 실패하면 즉시 생략. 느린 실패 56회 → 최대 1회분.
- **수집 순서 재배열**: 빠르고 값싼 소스부터(RSS → GOV.UK → FedReg → EUR-Lex → CBP → Gemini)
  + 단계별 소요시간 로그.
- 예산 270→330초, CBP 요청 56→28개(주간 창 기준).

## v5.0 — 주간 운영 최적화 (시간·비용 효율화)

매주 월요일 오전, 최근 일주일치 발행 체제에 맞춰 전면 재조정.

- **조회기간 7일**: 주간 발행 주기와 정합 (dedup이 있어 경계 누락 없음).
- **실행 순서 반전**: 무료·고속 직수집(CBP/FedReg/EUR-Lex/GOV.UK/RSS 14피드)을 먼저 확보하고,
  시간이 많이 드는 Gemini는 남은 시간예산으로 — **시간 초과가 나도 직수집분은 반드시 발송**됩니다.
  배치 단위 시간예산 가드로 Gemini가 예산을 넘기면 나머지 배치만 생략.
- **비용 -56%**: Gemini 그라운딩 패스 27 → **12개 통합** (한국/일본/중국/인도 단독 유지, 나머지는
  국가별 검색 전략을 내장한 통합 그룹 패스 — 40개국 커버 유지). 출력 토큰 상한 16384→8192.
- **균형**: 미국 CROSS 주간 총량 캡(80건) — 직수집 정상화 후 미국 편중 방지. 번역 상한 50콜.
- 예상 실행시간: 직수집 ~40초 + Gemini 2배치 ~2분 + 검증·번역 ~1분 = **약 3~4분** (6분 한도 내 안정).
